...
int32 OS_rename (const char *oldFile, const char *newFile)
{
    int status,i;
    ...
    */
    if (strlen(oldFile) >= OS_MAX_PATH_LEN)
        return OS_FS_ERR_PATH_TOO_LONG;
    
    if (strlen(newFile) >= OS_MAX_PATH_LEN)
        return OS_FS_ERR_PATH_TOO_LONG;

    /* 
    ** check if the names of the files are too long 
    */
    if (OS_check_name_length(oldFile) != OS_FS_SUCCESS)
        return OS_FS_ERR_NAME_TOO_LONG;

    if (OS_check_name_length(newFile) != OS_FS_SUCCESS)
        return OS_FS_ERR_NAME_TOO_LONG;
    ...
    ** Translate the path
    */
    if ( OS_TranslatePath(oldFile, (char *)old_path) != OS_FS_SUCCESS )
    {
        return OS_FS_ERR_PATH_INVALID;
    }
    ...
    ** Translate the path
    */
    if ( OS_TranslatePath(newFile, (char *)new_path) != OS_FS_SUCCESS )
    {
        return OS_FS_ERR_PATH_INVALID;
    }
    ...
    if (status == 0)
    {
        for ( i =0; i < OS_MAX_NUM_OPEN_FILES; i++) 
        {
            if (strcmp(OS_FDTable[i].Path, oldFile) == 0 &&
                OS_FDTable[i].IsValid == TRUE)
            {
                strncpy (OS_FDTable[i].Path, newFile, OS_MAX_PATH_LEN);
            } 
        }
        return OS_FS_SUCCESS;
    }
